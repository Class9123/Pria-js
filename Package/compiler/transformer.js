...
class DirectivePluginManager {
  ...
  register(plugin) {
    if (!plugin || !plugin.directive || typeof plugin.transform !== "function") {
...
class Transformer {
  ...
  emitAttributeBindings(path, { targetRef = null, includeDirectives = false } = {}) {
    const attrs = path.node.openingElement.attributes || [];
  ...
  renderElement(path, options = {}) {
    const { includeDirectives = false, targetRef = null, processChildren = true } =
      options;
